class LoanEventObserver {
  ...
  class LoanRepaymentObserver extends LoanEventObserver {
    update(event) {
      if (event.type === 'loanCreated') {
        ...
        // Aici poți să actualizezi UI-ul pentru a reflecta noile împrumuturi
      } else if (event.type === 'loansUpdated') {
        console.log('Împrumuturi actualizate:', event.data);
        // Aici poți să actualizezi UI-ul pentru a reflecta împrumuturile actualizate
      }
    }
  }
  ...
  App = {
    ...
    renderLoans: async () => {
      try {
        ...
        // Notifică observatorii despre împrumuturi
    ...
    createLoans: async () => {
      try {
        ...
        // Notifică observatorii despre crearea unui împrumut
  ...
  $(() => {
    $(window).on("load", () => {
      ...
      // Crearea unui observator și adăugarea acestuia în App
